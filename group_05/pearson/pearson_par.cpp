...
int main(int argc, char const* argv[])
{
    ...
    int blockSize = dimension/num_threads/2; //creating twice as many blocks as threads
    std::vector<ThreadData*> thread_data; 

    //the calculations for each block can be seen as a series of number 128, 127, 126, ..., 3, 2, 1
    //so splitting the calculations in the middle and flipping one side gives the number series: 129, 129, 129, ..., 129
    //ive used the same logic for dividing the number work for each thread with the blocks
    for(int thread_num = 0; thread_num < num_threads; thread_num++)
    {
        setstart = blockSize * thread_num; //start of first block for thread 
        setend = setstart + blockSize + 1;  //end of first block for thread
        secondstart = dimension - blockSize - blockSize*thread_num; //start of second block for thread
        secondend = dimension - blockSize*thread_num; //end of second block for thread
        if(thread_num != 0)
            secondend +=1;
